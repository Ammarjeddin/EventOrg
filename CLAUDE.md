...
- Each topic's `items` are tasks `{t, done, who, subs?, _open?}`. **Every task in every topic** can have a flat `subs` array of subtasks `{t, done, who}` — expand via the `▸ subtasks` chip (toggles `_open`). The generic subtask UI uses `data-gsub*`/`data-gstedit`/`data-gswho`/`data-gsubadd`/`data-exp="key|idx"` attributes (handlers in `wireDetail`); `subs` is created lazily on first add and every access guards with `|| []`, so **no migration is needed** for old boards.
- **The `content` key is an additional special case**: it's rendered "wide" and its tasks *also* carry `vis`/`aud` subtask arrays (Visuals / Audio, `CONTENT_CATS`) rendered as extra panes below the generic `subs` list. Content's vis/aud use the older `data-subcheck`/`data-stedit`/`data-swho`/`data-subadd` attributes (`idx|cat|sidx`) — keep them distinct from the generic `data-gsub*` ones.
- `_open` is in-memory expand/collapse state, **stripped from everything persisted** by `cleanData()` (which now walks all topics' items, not just content).
...
- `renderPeople()` — `collectAssignments()` walks every event/topic/item/subtask into a per-person map plus an "unassigned" bucket. Each `rowHtml` row is a two-line layout (`.prow` grid): checkbox + full-width task text on line 1, an `.pmeta` subline (event · team + `dayTag`) on line 2 — task text gets the width, metadata stays muted below.
